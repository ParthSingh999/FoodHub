...
export const Route = createFileRoute("/wallet")({
  head: () => ({
    meta: [
      { title: "Wallet & Payouts — Zamato Partner" },
      { name: "description", content: "Earnings, payouts and transaction history." },
    ],
  }),
  ...
  return (
    <AppShell title="Wallet & Payouts" subtitle="Your earnings, settlements and transactions">
      <div className="grid gap-4 md:grid-cols-3">
        <Card
          className="md:col-span-2 overflow-hidden border-none text-primary-foreground shadow-[var(--shadow-glow)]"
            ...
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-primary-foreground/80">
                  Pending settlement: ₹{walletPending.toLocaleString()}
                </p>
              </div>
              <div className="rounded-xl bg-white/15 px-3 py-2 text-xs font-medium backdrop-blur">
                HDFC ****4521
              </div>
            </div>
      ...
      <Card className="mt-6 border-border/60 shadow-[var(--shadow-card)]">
        <CardHeader>
          <CardTitle className="font-display">Earnings trend</CardTitle>
        </CardHeader>
        <CardContent>
          ...
          <div className="divide-y divide-border">
            {walletTxns.map((t) => (
              <div key={t.id} className="flex items-center gap-3 px-6 py-4">
                <div
                  className={`flex h-10 w-10 items-center justify-center rounded-full ${t.type === "credit" ? "bg-success/15 text-success" : "bg-destructive/10 text-destructive"}`}
                >
                  {t.type === "credit" ? (
                ...
                <p
                  className={`font-display font-bold ${t.type === "credit" ? "text-success" : "text-foreground"}`}
                >
