...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Monthly revenue, orders, and performance overview for your restaurant.",
      },
    ],
  }),
  component: Dashboard,
    ...
    >
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        {stats.map((s) => (
          <Card
            key={s.label}
            className="overflow-hidden border-border/60 shadow-[var(--shadow-card)]"
          >
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={260}>
              <PieChart>
                ...
                >
                  {categoryMix.map((_, i) => (
                    <Cell key={i} fill={chartColors[(i % 5) + 1] || "var(--color-chart-1)"} />
                  ))}
                </Pie>
                <Tooltip
                  contentStyle={{
...
function StatusBadge({ status }: { status: keyof typeof statusLabel }) {
  ...
  return (
    <span className={`rounded-full border px-2.5 py-0.5 text-xs font-medium ${map[status]}`}>
      {statusLabel[status]}
    </span>
  );
}
