...
export const Route = createFileRoute("/customers")({
  head: () => ({
    meta: [
      { title: "Customers — Zamato Partner" },
      { name: "description", content: "Details of customers who ordered from your restaurant." },
    ],
  }),
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody className="divide-y divide-border">
                {customers.map((c) => {
                  const tier =
                    c.totalSpent > 15000 ? "Gold" : c.totalSpent > 5000 ? "Silver" : "New";
                  return (
                    <tr key={c.id} className="text-sm transition hover:bg-muted/30">
                      ...
                      <td className="px-4 py-3 text-xs text-muted-foreground">
                        {new Date(c.lastOrder).toLocaleDateString("en-IN", { dateStyle: "medium" })}
                      </td>
                      <td className="px-4 py-3">
                        <Badge
                          className={
